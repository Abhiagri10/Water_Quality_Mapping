...
// Assets     : temptCombined | Chao_Phraya | Khlong_Nueng (user-defined GEE assets)
// =============================================================================
...
function calculateIndices(image) {
  ...
  var B5_B6_B7 = image.select('SR_B5').add(image.select('SR_B6')).add(image.select('SR_B7')).rename('B5_B6_B7');
...
function prepareTrainingData(landsatImage, temptFeatureCollection) {
  ...
  var dateFilteredTempt = temptFeatureCollection.filter(ee.Filter.equals('date', imageDate));
  ...
  var trainingPoints = samplingPoints.map(function(point) {
    ...
    return ee.Feature(point.geometry(), {
      ...
      'NIR': values.get('NIR'),
      'Tempt': ee.Number(ee.Algorithms.If(
        matchingTempt,
        matchingTempt.get('Tempt'),
        null
      )),
...
Object.keys(waterBodies).forEach(function(waterBodyName) {
  ...
  predictedImagesYear.size().evaluate(function(size, error) {
    if (error) {
      ...
      print('Initiating monthly average calculations for Tempt in ' + waterBodyName + '...');
    }
  });
  ...
  monthsArray.forEach(function(month) {
    ...
    monthlyCollection.size().evaluate(function(numberOfImages) {
      if (numberOfImages > 0) {
        ...
        var monthlyMeanImage = monthlyStack.reduce('mean').rename('Tempt')
          .set('month', startDate.format('YYYY-MM'))
          .set('water_body', waterBodyName);

        var exportName = waterBodyName + '_Tempt_MonthlyAvg_' + startDate.format('YYYY-MM').getInfo();
        ...
        Export.image.toDrive({
          ...
          folder: 'Landsat8_9_Tempt_Monthly_Avg_Prediction_Images_' + predictionYearString,
          scale: 30,
          crs: 'EPSG:4326',
          region: waterBody,
          fileFormat: 'GeoTIFF',
...
if (chaoPhrayaMonthlyMeanVis) {
  Map.addLayer(chaoPhrayaMonthlyMeanVis, visParams, 'Chao_Phraya_Tempt_' + visualizationMonth);
}
...
if (khlongNuengMonthlyMeanVis) {
  Map.addLayer(khlongNuengMonthlyMeanVis, visParams, 'Khlong_Nueng_Tempt_' + visualizationMonth);
}
