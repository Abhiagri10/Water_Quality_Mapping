...
// Split      : ~84% training / ~5% validation / ~11% testing
// Indices    : B2_B4, B2_B3, B2_B7, B6_B7
// Asset      : salinityCombined  (GEE FeatureCollection)
// =============================================================================
...
function prepareTrainingData(landsatImage, salinityFC) {
  ...
  var dateFilteredSalinity = salinityFC.filter(ee.Filter.equals('date', imageDate));
  ...
  var trainingPoints = samplingPoints.map(function(point) {
    ...
    return ee.Feature(point.geometry(), {
      ...
      'Salinity': ee.Number(ee.Algorithms.If(matchingSalinity, matchingSalinity.get('Salinity'), null)),
...
var salinityCombined = ee.FeatureCollection('projects/ee-st124278/assets/WQ_Combined/salinityCombined');
...
var validation = withRandom.filter(ee.Filter.and(
  ee.Filter.gte('random', 0.84), ee.Filter.lt('random', 0.89)
));
...
// 500 trees — Salinity-specific optimisation
...
function calculatePerformanceMetrics(predictions) {
  var errorFeatures = predictions.map(function(feature) {
    ...
    var observed  = ee.Number(feature.get('Salinity'));
  ...
  var r2   = ee.Number(errorFeatures.reduceColumns({
    ...
    selectors: ['classification', 'Salinity']
...
function createValidationPlot(predictions, title, metrics) {
  ...
  .setOptions({
    ...
    hAxis    : {title: 'Observed Salinity (ppm)'},
    vAxis    : {title: 'Predicted Salinity (ppm)'},
