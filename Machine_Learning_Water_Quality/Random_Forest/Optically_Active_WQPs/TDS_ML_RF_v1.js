...
// Split      : ~71% training / ~21% validation / ~8% testing
// Indices    : B2_B4, B2_B3, B2_B7, C
// Asset      : tdsCombined  (GEE FeatureCollection)
// =============================================================================
...
function prepareTrainingData(landsatImage, tdsFC) {
  ...
  var dateFilteredTDS = tdsFC.filter(ee.Filter.equals('date', imageDate));
  ...
  var trainingPoints = samplingPoints.map(function(point) {
    ...
    return ee.Feature(point.geometry(), {
      ...
      'C'      : values.get('C'),
      'TDS'    : ee.Number(ee.Algorithms.If(matchingTDS, matchingTDS.get('TDS'), null)),
...
var tdsCombined = ee.FeatureCollection('projects/ee-st124278/assets/WQ_Combined/tdsCombined');
...
var validation = withRandom.filter(ee.Filter.and(
  ee.Filter.gte('random', 0.71), ee.Filter.lt('random', 0.92)
));
...
function calculatePerformanceMetrics(predictions) {
  var errorFeatures = predictions.map(function(feature) {
    ...
    var observed  = ee.Number(feature.get('TDS'));
...
function createValidationPlot(predictions, title, metrics) {
  ...
  .setOptions({
    ...
    hAxis    : {title: 'Observed TDS (mg/L)'},
    vAxis    : {title: 'Predicted TDS (mg/L)'},
