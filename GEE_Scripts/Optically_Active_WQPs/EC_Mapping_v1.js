...
// Parameter  : Electrical Conductivity — EC (µS/cm)
...
// Assets     : ecCombined | Chao_Phraya | Khlong_Nueng (user-defined GEE assets)
// =============================================================================
...
function prepareTrainingData(landsatImage, ecFeatureCollection) {
  ...
  var dateFilteredEC = ecFeatureCollection.filter(ee.Filter.equals('date', imageDate));
...
Object.keys(waterBodies).forEach(function(waterBodyName) {
  ...
  predictedImagesYear.size().evaluate(function(size, error) {
    if (error) {
      ...
      print('Initiating monthly average calculations for EC in ' + waterBodyName + '...');
    }
  });
  ...
  monthsArray.forEach(function(month) {
    ...
    monthlyCollection.size().evaluate(function(numberOfImages) {
      if (numberOfImages > 0) {
        ...
        var monthlyMeanImage = monthlyStack.reduce('mean').rename('EC')
          .set('month', startDate.format('YYYY-MM'))
          .set('water_body', waterBodyName);

        var exportName = waterBodyName + '_EC_MonthlyAvg_' + startDate.format('YYYY-MM').getInfo();
        ...
        Export.image.toDrive({
          ...
          folder: 'Landsat8_9_EC_Monthly_Avg_Prediction_Images_' + predictionYearString,
          scale: 30,
          crs: 'EPSG:4326',
          region: waterBody,
          fileFormat: 'GeoTIFF',
...
if (chaoPhrayaMonthlyMeanVis) {
  Map.addLayer(chaoPhrayaMonthlyMeanVis, visParams, 'Chao_Phraya_EC_' + visualizationMonth);
}
...
if (khlongNuengMonthlyMeanVis) {
  Map.addLayer(khlongNuengMonthlyMeanVis, visParams, 'Khlong_Nueng_EC_' + visualizationMonth);
}
