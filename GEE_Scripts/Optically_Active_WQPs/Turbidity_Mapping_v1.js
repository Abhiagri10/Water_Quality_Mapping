...
// Assets     : turbidityCombined | Chao_Phraya | Khlong_Nueng (user-defined GEE assets)
// =============================================================================
...
function prepareTrainingData(landsatImage, turbidityFeatureCollection) {
  ...
  var dateFilteredTurbidity = turbidityFeatureCollection.filter(ee.Filter.equals('date', imageDate));
  ...
  var trainingPoints = samplingPoints.map(function(point) {
    ...
    return ee.Feature(point.geometry(), {
      ...
      'Turbidity': ee.Number(ee.Algorithms.If(
        matchingTurbidity,
...
allFeatures = allFeatures.filter(ee.Filter.notNull(['Turbidity', 'B3_B4', 'B4_B7', 'NDTI', 'NDVI']));
...
Object.keys(waterBodies).forEach(function(waterBodyName) {
  ...
  predictedImagesYear.size().evaluate(function(size, error) {
    if (error) {
      ...
      print('Initiating monthly average calculations for Turbidity in ' + waterBodyName + '...');
    }
  });
  ...
  monthsArray.forEach(function(month) {
    ...
    monthlyCollection.size().evaluate(function(numberOfImages) {
      if (numberOfImages > 0) {
        ...
        var monthlyMeanImage = monthlyStack.reduce('mean').rename('Turbidity')
          .set('month', startDate.format('YYYY-MM'))
          .set('water_body', waterBodyName);

        var exportName = waterBodyName + '_Turbidity_MonthlyAvg_' + startDate.format('YYYY-MM').getInfo();
        ...
        Export.image.toDrive({
          ...
          folder: 'Landsat8_9_Turbidity_Monthly_Avg_Prediction_Images_' + predictionYearString,
          scale: 30,
          crs: 'EPSG:4326',
          region: waterBody,
          fileFormat: 'GeoTIFF',
...
if (chaoPhrayaMonthlyMeanVis) {
  Map.addLayer(chaoPhrayaMonthlyMeanVis, visParams, 'Chao_Phraya_Turbidity_' + visualizationMonth);
}
...
if (khlongNuengMonthlyMeanVis) {
  Map.addLayer(khlongNuengMonthlyMeanVis, visParams, 'Khlong_Nueng_Turbidity_' + visualizationMonth);
}
