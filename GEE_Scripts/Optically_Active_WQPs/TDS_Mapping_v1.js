...
// Parameter  : Total Dissolved Solids — TDS (mg/L)
...
// Assets     : tdsCombined | Chao_Phraya | Khlong_Nueng (user-defined GEE assets)
// =============================================================================
...
// Spectral indices calculation for TDS
...
function prepareTrainingData(landsatImage, tdsFeatureCollection) {
  ...
  var dateFilteredTDS = tdsFeatureCollection.filter(ee.Filter.equals('date', imageDate));
  ...
  var trainingPoints = samplingPoints.map(function(point) {
    ...
    return ee.Feature(point.geometry(), {
      ...
      'C': values.get('C'),
      'TDS': ee.Number(ee.Algorithms.If(
        matchingTDS,
        matchingTDS.get('TDS'),
        null
      )),
  ...
  return trainingPoints.filter(ee.Filter.notNull(['TDS']));
}
...
var allFeatures = filteredCollection.map(function(image) {
  ...
  return prepareTrainingData(image, dateFilteredTDS)
...
allFeatures = allFeatures.filter(ee.Filter.notNull(['TDS', 'B2_B4', 'B2_B3', 'B2_B7', 'C']));

var seed = 42;
var withRandom = allFeatures.randomColumn('random', seed);

var training = withRandom.filter(ee.Filter.lt('random', 0.71));
var validation = withRandom.filter(ee.Filter.and(
  ee.Filter.gte('random', 0.71),
  ee.Filter.lt('random', 0.92)
));
...
Object.keys(waterBodies).forEach(function(waterBodyName) {
  ...
  predictedImagesYear.size().evaluate(function(size, error) {
    if (error) {
      ...
      print('Initiating monthly average calculations for TDS in ' + waterBodyName + '...');
    }
  });
  ...
  monthsArray.forEach(function(month) {
    ...
    monthlyCollection.size().evaluate(function(numberOfImages) {
      if (numberOfImages > 0) {
        ...
        var monthlyMeanImage = monthlyStack.reduce('mean').rename('TDS')
          .set('month', startDate.format('YYYY-MM'))
          .set('water_body', waterBodyName);

        var exportName = waterBodyName + '_TDS_MonthlyAvg_' + startDate.format('YYYY-MM').getInfo();
        ...
        Export.image.toDrive({
          ...
          folder: 'Landsat8_9_TDS_Monthly_Avg_Prediction_Images_' + predictionYearString,
          scale: 30,
          crs: 'EPSG:4326',
          region: waterBody,
          fileFormat: 'GeoTIFF',
...
var visParams = {
  min: 0,
  max: 200,
  palette: ['006600', '009900', '00CC00', '00FF00', '99FF00',
            'CCFF00', 'FFFF00', 'FFCC00', 'FF9900', 'FF6600', 'FF0000']
};
...
if (chaoPhrayaMonthlyMeanVis) {
  Map.addLayer(chaoPhrayaMonthlyMeanVis, visParams, 'Chao_Phraya_TDS_' + visualizationMonth);
}
...
if (khlongNuengMonthlyMeanVis) {
  Map.addLayer(khlongNuengMonthlyMeanVis, visParams, 'Khlong_Nueng_TDS_' + visualizationMonth);
}
