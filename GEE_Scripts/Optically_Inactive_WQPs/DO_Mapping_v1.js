...
// Parameter  : Dissolved Oxygen — DO (mg/L)
// Model      : Random Forest (smileRandomForest, 100 trees)
// Output     : Monthly mean maps — Chao Phraya & Khlong Nueng (2024)
// Indices    : SWIR1, B6_B7  [+ in-situ TDS, Turbidity as hybrid features]
// Assets     : doCombined | tdsCombined | turbidityCombined | Chao_Phraya | Khlong_Nueng
// =============================================================================
...
function prepareTrainingData(landsatImage, doFeatureCollection, tdsCollection, turbidityCollection) {
  ...
  var dateFilteredDO = doFeatureCollection.filter(ee.Filter.equals('date', imageDate));
  ...
  return trainingPoints.filter(ee.Filter.notNull(['DO', 'tdsCombined', 'turbidityCombined']));
}
...
var allFeatures = filteredCollection.map(function(image) {
  ...
  var dateFilteredDO = doCombined.filter(ee.Filter.equals('date', imageDate));
  ...
  var matchedPoints = samplingPoints
    .filter(ee.Filter.inList('Station', dateFilteredDO.aggregate_array('station')))
    .filter(ee.Filter.inList('Zone', dateFilteredDO.aggregate_array('zone')))
  ...
  return prepareTrainingData(image, dateFilteredDO, dateFilteredTDS, dateFilteredTurbidity)
...
allFeatures = allFeatures.filter(ee.Filter.notNull(['DO', 'tdsCombined', 'turbidityCombined', 'SWIR1', 'B6_B7']));
...
function standardizeFeatures(trainingSet, featureCollections) {
  var features = ['tdsCombined', 'turbidityCombined', 'SWIR1', 'B6_B7'];
  var means = {};
  var stdDevs = {};
...
var trainedModel = ee.Classifier.smileRandomForest(100)
  ...
  .train({
    ...
    classProperty: 'DO',
    inputProperties: ['tdsCombined', 'turbidityCombined', 'SWIR1', 'B6_B7']
  });
...
Object.keys(waterBodies).forEach(function(waterBodyName) {
  ...
  predictedImagesYear.size().evaluate(function(size, error) {
    if (error) {
      ...
      print('Initiating monthly average calculations for DO in ' + waterBodyName + '...');
    }
  });
  ...
  monthsArray.forEach(function(month) {
    ...
    monthlyCollection.size().evaluate(function(numberOfImages) {
      if (numberOfImages > 0) {
        ...
        var monthlyMeanImage = monthlyStack.reduce('mean').rename('DO')
          .set('month', startDate.format('YYYY-MM'))
          .set('water_body', waterBodyName);

        var monthStr = month < 10 ? '0' + month : month.toString();
        var exportName = waterBodyName + '_DO_MonthlyAvg_' + predictionYear + '-' + monthStr;
        ...
        Export.image.toDrive({
          ...
          folder: 'Landsat8_9_DO_Monthly_Avg_Prediction_Images_' + predictionYearString,
          scale: 30,
          crs: 'EPSG:4326',
          region: waterBody,
          fileFormat: 'GeoTIFF',
...
if (chaoPhrayaMonthlyMeanVis) {
  Map.addLayer(chaoPhrayaMonthlyMeanVis, visParams, 'Chao_Phraya_DO_' + visualizationMonth);
}
...
if (khlongNuengMonthlyMeanVis) {
  Map.addLayer(khlongNuengMonthlyMeanVis, visParams, 'Khlong_Nueng_DO_' + visualizationMonth);
}
